...
namespace Overlewd
{
    ...
    {
        public abstract class InfoBlockEquipBase : BaseInfoBlock
        {
            public List<int> consumeEquips { get; private set; } = new List<int>();

            protected void AutoFilled()
            {
            ...
            public virtual AdminBRO.ForgePrice.MergeEquipmentSettings mergeSettings => null;
            public int[] mergeIds => consumeEquips.GetRange(0, equipsNeeded).ToArray();
            public bool IsConsume(EquipmentBase equip) =>
                consumeEquips.Exists(eId => eId == equip.equipId);
            public void Consume(EquipmentBase equip)
            {
                if (isFilled)
                {
                    var consEquipData = consumeEquipData;
            ...
            public void RemoveConsume(EquipmentBase equip)
            {
